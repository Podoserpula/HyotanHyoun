...
import AccessaryCustomizer from './TabContents/AccessaryCustomizer';
...
const accessaries = [
	...
	},
	{
		...
		title: "アンテナ",
		svg: (<svg viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path d="M150 63L150 28" stroke="#222222" strokeWidth="3" strokeLinecap="round" />
			<circle cx="150" cy="28" r="9.5" fill="#FAFAFA" stroke="#222222" strokeWidth="3" />
		</svg>
		),
	},
	{
		...
		title: "ミニひょうたんひょうん",
		svg: (<svg viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path d="M150.059 67.6198C154.737 67.6198 158.223 66.8507 160.69 65.4542C163.212 64.0265 164.605 61.9728 165.065 59.6462C165.512 57.3825 165.051 55.0003 164.167 52.848C163.278 50.6843 161.918 48.6419 160.408 46.9994C159.222 45.7101 158.805 44.5269 158.749 43.4078C158.691 42.2343 159.024 40.9894 159.597 39.5855C159.881 38.8889 160.209 38.1856 160.558 37.4484C160.59 37.3807 160.623 37.3127 160.655 37.2443C160.97 36.5804 161.298 35.887 161.595 35.1926C162.246 33.6718 162.821 31.9816 162.821 30.2083C162.821 26.7916 161.369 24.2605 158.92 22.6542C156.563 21.1082 153.414 20.5 150.059 20.5C146.695 20.5 143.535 21.1368 141.169 22.6922C138.719 24.3032 137.242 26.8271 137.242 30.2083C137.242 31.9257 137.735 33.5267 138.288 34.9703C138.528 35.5958 138.763 36.1592 138.99 36.7011C139.327 37.5094 139.645 38.2698 139.927 39.1168C140.367 40.4379 140.6 41.6717 140.461 42.9199C140.323 44.1474 139.811 45.5135 138.553 47.0732C135.594 50.7423 133.56 55.7952 134.939 60.1475C136.395 64.7426 141.284 67.6198 150.059 67.6198Z" fill="#FAFAFA" stroke="#222222" strokeWidth="3" />
			<path d="M159.204 31.7842C157.293 28.2447 154.054 27.5135 152.07 31.5226" stroke="#222222" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
			<path d="M148.133 31.7842C146.222 28.2447 142.984 27.5135 141 31.5226" stroke="#222222" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
		</svg>
	...
	},
	{
		...
		title: "リボン",
		svg: (<svg viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path d="M146.532 52.3054C146.532 52.3054 122.101 81.8238 115.332 69.639C112.045 63.7215 116.29 59.0065 115.333 52.3054C114.122 43.829 106.146 39.766 108.399 31.5051C112.46 16.6128 146.532 41.9052 146.532 41.9052V52.3054Z" fill="#FAFAFA" stroke="#222222" strokeWidth="3" strokeLinejoin="round" />
			<path d="M153.468 52.3054C153.468 52.3054 177.899 81.8238 184.668 69.639C187.955 63.7215 183.71 59.0065 184.667 52.3054C185.878 43.829 193.854 39.766 191.601 31.5051C187.54 16.6128 153.468 41.9052 153.468 41.9052V52.3054Z" fill="#FAFAFA" stroke="#222222" strokeWidth="3" strokeLinejoin="round" />
			<path d="M129.2 48.8386H170.801" stroke="#222222" strokeWidth="3" strokeLinejoin="round" />
			<path d="M143.066 55.7721V41.9053H156.933V55.7721H143.066Z" fill="#FAFAFA" stroke="#222222" strokeWidth="3" strokeLinejoin="round" />
		</svg>
	...
	},
	{
		...
		title: "帽子",
		svg: (<svg viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path d="M185.166 50.6038C193.64 58.99 192.988 75 192.988 75C192.988 75 173.941 65.2817 150.149 65.2817C126.357 65.2817 106.295 75 106.295 75C106.295 75 103.701 59.5491 113.901 50.6038C122.586 42.9875 129.777 43.6257 137.908 33.8314C144.102 26.3707 150.149 6 150.149 6C150.149 6 155.408 26.3534 161.374 33.8314C168.601 42.8883 176.692 42.2176 185.166 50.6038Z" fill="#D9D9D9" stroke="#222222" strokeWidth="3" strokeLinejoin="round" />
		</svg>
...
export default function ImageSwapu() {
	...
	const [selectedAccessary, setSlectedAccessary] = useState(accessaries[0]);
	const [accessaryColor, setAccessaryColor] = useState('#FAFAFA');

	const handleAccessarySelect = (image: typeof accessaries[0], index: number) => {
		const currentIndex = accessaries.findIndex((img) => img.id === selectedAccessary.id);
		setDirection(index > currentIndex ? 1 : -1);
		setSlectedAccessary(image);
	};

	const handleAccessaryColorChange = (event: React.ChangeEvent<HTMLInputElement>) => {
		setAccessaryColor(event.target.value);
	};
	...
	return (
		<div className="flex flex-col space-y-4">
			<Preview
				...
				direction={direction}
				selectedAccessary={selectedAccessary}
				accessaryColor={accessaryColor}
			/>
			...
			<div className="flex space-x-4">
				...
				<button
					onClick={() => setActiveTab('accessary')}
					className={`px-5 py-2 rounded ${activeTab === 'accessary' ? 'bg-blue-500 text-white' : 'bg-gray-200'
						}`}
				>
					<div className="flex flex-col items-center"><svg viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M147.147 283.346C176.414 283.346 197.16 278.868 211.223 271.349C225.389 263.774 232.73 253.125 235.036 241.15C237.319 229.3 234.618 216.454 229.354 204.402C224.079 192.326 216.14 180.837 207.634 171.588C199.615 162.869 196.148 154.467 195.315 146.253C194.475 137.97 196.289 129.657 199.223 121.089C200.689 116.81 202.416 112.516 204.205 108.153C204.461 107.528 204.719 106.902 204.977 106.274C206.513 102.537 208.072 98.7464 209.491 94.941C212.806 86.0554 215.465 76.8414 215.465 67.337C215.465 48.4465 208.019 35.7671 195.453 27.9613C183.097 20.286 166.077 17.5 147.147 17.5C127.839 17.5 111.375 20.7943 99.6736 28.7592C87.8021 36.8402 81.1625 49.5029 81.1625 67.337C81.1625 76.3627 83.7599 85.0041 86.9653 93.3716C88.2397 96.6983 89.5843 99.9263 90.9177 103.127C93.0147 108.162 95.0841 113.13 96.8091 118.311C99.5788 126.63 101.249 134.97 100.275 143.681C99.3053 152.357 95.6888 161.61 87.5429 171.711C70.554 192.777 58.972 220.508 65.2082 243.352C68.3611 254.902 76.0223 264.977 89.3796 272.106C102.664 279.196 121.518 283.346 147.147 283.346Z" fill="#FAFAFA" stroke="#222222" strokeWidth="9" />
						...
						<path d="M182.767 83.8263C190.059 83.8263 196.779 82.575 201.769 80.4543C204.26 79.3959 206.436 78.0717 208.026 76.4737C209.627 74.8655 210.757 72.8421 210.757 70.4935C210.757 68.1448 209.627 66.1214 208.026 64.5133C206.436 62.9153 204.26 61.591 201.769 60.5327C196.779 58.4119 190.059 57.1606 182.767 57.1606C175.474 57.1606 168.754 58.4119 163.764 60.5327C161.274 61.591 159.098 62.9153 157.507 64.5133C155.907 66.1214 154.776 68.1448 154.776 70.4935C154.776 72.8421 155.907 74.8655 157.507 76.4737C159.098 78.0717 161.274 79.3959 163.764 80.4543C168.754 82.575 175.474 83.8263 182.767 83.8263Z" fill="FAFAFA" stroke="#222222" strokeWidth="9" />
						<ellipse cx="182.767" cy="70.6458" rx="12.3812" ry="14.342" fill="#222222" />
						<circle cx="150" cy="20" r="15" fill="#FAFAFA" stroke="#ff8e00" strokeWidth="9" />
					</svg>
			...
			{activeTab === 'leftEye' && (
				<EyesCustomizer
					...
					onIrisSizeChange={handleLeftIrisSizeChange} />
			)}
			...
			{activeTab === 'rightEye' && (
				<EyesCustomizer
					...
					onIrisSizeChange={handleRightIrisSizeChange} />
			)}
			...
			{activeTab === 'accessary' && (
				<AccessaryCustomizer
					...
					selectedAccessary={selectedAccessary}
					accessaryColor={accessaryColor}
					onAccessarySelect={handleAccessarySelect}
					onAccessaryColorChange={handleAccessaryColorChange}
				/>
			)}
		</div>
	);
